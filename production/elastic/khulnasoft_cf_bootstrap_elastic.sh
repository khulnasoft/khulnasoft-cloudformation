...
node_name=$(cat /tmp/khulnasoft_cf_settings | grep '^NodeName:' | cut -d' ' -f2)
master_ip=$(cat /tmp/khulnasoft_cf_settings | grep '^MasterIp:' | cut -d' ' -f2)
worker_ip=$(cat /tmp/khulnasoft_cf_settings | grep '^WorkerIp:' | cut -d' ' -f2)
kibana_ip=$(cat /tmp/khulnasoft_cf_settings | grep '^KibanaIp:' | cut -d' ' -f2)
...
load_template(){
    ...
    url_alerts_template="https://raw.githubusercontent.com/khulnasoft/khulnasoft/${TAG}/extensions/elasticsearch/7.x/khulnasoft-template.json"
    alerts_template="/tmp/khulnasoft-template.json"
    curl -Lo ${alerts_template} ${url_alerts_template}
    curl -XPUT "https://${eth0_ip}:9200/_template/khulnasoft" -k -u elastic:${ssh_password} -H 'Content-Type: application/json' -d@${alerts_template}
    curl -XDELETE "https://${eth0_ip}:9200/khulnasoft-alerts-*" -k -u elastic:${ssh_password}
...
EOF
  ...
  while true;
  do
    CODE=`curl -w '%{response_code}' -s -o /dev/null -i -XPOST "https://$eth0_ip:9200/_security/user/khulnasoft" -k -u elastic:${ssh_password} -d@${user_config} -H 'Content-Type: application/json'`
    if [ $CODE == '200' ]; then
      echo "User khulnasoft created" >> /tmp/deploy.log
      break
    fi
  done
}
