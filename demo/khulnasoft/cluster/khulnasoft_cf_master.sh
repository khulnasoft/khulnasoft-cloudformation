...
splunk_ip=$(cat /tmp/khulnasoft_cf_settings | grep '^SplunkIP:' | cut -d' ' -f2)
WindowsPublicIp=$(cat /tmp/khulnasoft_cf_settings | grep '^WindowsPublicIp:' | cut -d' ' -f2)
...
then
  ...
  # Installing khulnasoft-api
  yum -y install khulnasoft-api-$khulnasoft_version
  chkconfig --add khulnasoft-api
  echo "Installed Khulnasoft API." >> /tmp/deploy.log
else
  API_BRANCH=$api_branch
  npm config set user 0
  curl -LO https://github.com/khulnasoft/khulnasoft-api/archive/$API_BRANCH.zip
  unzip $API_BRANCH.zip
  rm -f $API_BRANCH.zip
  cd khulnasoft-api-$API_BRANCH
  ./install_api.sh
fi
...
wget https://khulnasoft.com/resources/iplist-to-cdblist.py -O /tmp/iplist-to-cdblist.py
...
chown khulnasoft:khulnasoft /var/ossec/etc/lists/blacklist-alienvault
...
# Configuring Khulnasoft API user and password
cd /var/ossec/api/configuration/auth
node htpasswd -b -c user ${khulnasoft_api_user} ${khulnasoft_api_password}

# Enable Khulnasoft API SSL and configure listening port
...
sed -i "s/config.port = \"55000\";/config.port = \"${khulnasoft_api_port}\";/" /var/ossec/api/configuration/config.js
echo "Setting port and SSL to Khulnasoft API." >> /tmp/deploy.log

# Restart khulnasoft-api
systemctl restart khulnasoft-api
echo "Restarted Khulnasoft API." >> /tmp/deploy.log
