...
kibana_port=$(cat /tmp/khulnasoft_cf_settings | grep '^KibanaPort:' | cut -d' ' -f2)
...
cluster.name: "khulnasoft_elastic"
...
get_plugin_url(){
  ...
  plugin_url="https://s3-us-west-1.amazonaws.com/packages-dev.khulnasoft.com/pre-release/app/kibana/khulnasoftapp-${khulnasoft_major}.${khulnasoft_minor}.${khulnasoft_patch}_${elastic_major_version}.${elastic_minor_version}.${elastic_patch_version}.zip"
  elif [[ ${EnvironmentType} == 'production' ]]
  then
  plugin_url="https://packages.khulnasoft.com/khulnasoftapp/khulnasoftapp-${khulnasoft_major}.${khulnasoft_minor}.${khulnasoft_patch}_${elastic_major_version}.${elastic_minor_version}.${elastic_patch_version}.zip"
  elif [[ ${EnvironmentType} == 'devel' ]]
  then
  plugin_url="https://packages-dev.khulnasoft.com/staging/app/kibana/khulnasoftapp-${khulnasoft_major}.${khulnasoft_minor}.${khulnasoft_patch}_${elastic_major_version}.${elastic_minor_version}.${elastic_patch_version}.zip"
  elif [[ ${EnvironmentType} == 'sources' ]]
  then
    BRANCH="$khulnasoft_major.$khulnasoft_minor-$elastic_major_version.$elastic_minor_version"
    if [[ $BRANCH != "" ]]; then
      yum install -y git
      ...
      git clone https://github.com/khulnasoft/khulnasoft-kibana-app -b $BRANCH --single-branch --depth=1 app
      cd app
      yarn
      ...
      echo 'Error: Unsupported Khulnasoft Plugin installation method' >> /tmp/deploy.log
    fi
  else
...
install_plugin(){
  ...
  echo "Redirecting to Khulnasoft app " >> /tmp/deploy.log
  # Set Khulnasoft app as the default landing page
  echo "server.defaultRoute: /app/khulnasoft" >> /etc/kibana/kibana.yml
  # Redirect Kibana welcome screen to
  echo "Redirect Kibana welcome screen to Discover"
  sed -i "s:'/app/kibana#/home':'/app/khulnasoft':g" /usr/share/kibana/src/ui/public/chrome/directives/global_nav/global_nav.html
  sed -i "s:'/app/kibana#/home':'/app/khulnasoft':g" /usr/share/kibana/src/ui/public/chrome/directives/header_global_nav/header_global_nav.js
...
add_api(){
echo "Adding Khulnasoft API" >> /tmp/deploy.log
sed -ie '/- default:/,+4d' /usr/share/kibana/optimize/khulnasoft/config/khulnasoft.yml
cat > /usr/share/kibana/optimize/khulnasoft/config/khulnasoft.yml << EOF
hosts:
  - default:
      url: https://${khulnasoft_master_ip}
      port: ${khulnasoft_api_port}
      user: ${khulnasoft_api_user}
      password: ${khulnasoft_api_password}
EOF
...
{
  "changes": {
    "defaultIndex": "khulnasoft-alerts-3.x-*"
  }
}
EOF
